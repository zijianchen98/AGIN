...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_file', type=str,
                        default='F:/PHD/AIGC/AIGN_Database/AIGN/CoCosNet_97.png')  # 'F:/PHD/AIGC/AIGN_Database/AIGN/CoCosNet_55.png'
    parser.add_argument('--torchscript_file', type=str,
                        default='./deployment/pal4vst/swin-large_upernet_unified_512x512/end2end.pt')
    ...
    pink = np.zeros((img.shape))
    pink[:, :, 0] = 255
    pink[:, :, 2] = 255
    img_with_pal = img * (1 - pal_np[:, :, None]) + args.alpha * pink * pal_np[:, :, None] + (
            1 - args.alpha) * img * pal_np[:, :, None]
    print(img_with_pal)

    if args.half_precision:
        model = model.half()
        img_tensor = img_tensor.half()
    cv2.imshow('czj', img_with_pal.astype(np.uint8))
    cv2.imshow('out_pal', (pal_np * 255.0).astype(np.uint8))
    cv2.waitKey(0)
